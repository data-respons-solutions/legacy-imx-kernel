 ...
 *  - tsc2007.c
 *
 * Copyright (C) 2016 DataRespons AS
 * Author: Artur Siewierski <asi@datarespons.no>
 *
 *  - Added Device Tree Integrity
 *
...
#define CONFIG_TOUCHSCREEN_SPERRE

#ifdef CONFIG_TOUCHSCREEN_SPERRE

#define TOUCHSCREEN_SPERRE_XMIN			0
#define TOUCHSCREEN_SPERRE_XMAX			800
#define TOUCHSCREEN_SPERRE_YMIN			0
#define TOUCHSCREEN_SPERRE_YMAX			480
#define TOUCHSCREEN_SPERRE_XRES			800
#define TOUCHSCREEN_SPERRE_YRES			480
#define TOUCHSCREEN_SPERRE_TOUCH_MAX	240
...
#ifdef CONFIG_OF
static int tsc2004_get_pendown_state_gpio(void *handle);
static int tsc2004_probe_dt(struct i2c_client *client, struct tsc2004 *ts);
#endif
...
static void tsc2004_work(struct work_struct *work)
{
	...
	if (rt) {
		...
		// calibration
		tc.x = tc.x * TOUCHSCREEN_SPERRE_XMAX/TOUCHSCREEN_SPERRE_TOUCH_MAX;	// for x axis
		tc.y = tc.y * TOUCHSCREEN_SPERRE_YMAX/TOUCHSCREEN_SPERRE_TOUCH_MAX;	// for y axis

		//printk(KERN_CRIT "-------- | point(%4d,%4d)\n", tc.x, tc.y);
...
static int tsc2004_probe_dt(struct i2c_client *client, struct tsc2004 *ts)
{
	...
	if (gpio_is_valid(ts->gpio))
		ts->get_pendown_state = (void *)tsc2004_get_pendown_state_gpio(ts);
	else
	{
		dev_warn(&client->dev,
...
static int tsc2004_probe_pdev(struct i2c_client *client, struct tsc2004 *ts,
			      const struct tsc2004_platform_data *pdata,
			      const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, " %d | x_plate_ohms = %d\n", __LINE__, pdata->x_plate_ohms);
...
static int tsc2004_probe(struct i2c_client *client,
				   const struct i2c_device_id *id)
{
	const struct tsc2004_platform_data *pdata = dev_get_platdata(&client->dev);
	struct tsc2004 *ts;
	...
	ts = devm_kzalloc(&client->dev, sizeof(struct tsc2004), GFP_KERNEL);
	if (!ts)
		return -ENOMEM;
	...
	if (pdata)
		err = tsc2004_probe_pdev(client, ts, pdata, id);
	else
		err = tsc2004_probe_dt(client, ts);
...
#ifdef CONFIG_TOUCHSCREEN_SPERRE
	input_set_abs_params(input_dev, ABS_X, 0, TOUCHSCREEN_SPERRE_XRES, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, TOUCHSCREEN_SPERRE_YRES, 0, 0);
#else
...
MODULE_AUTHOR("Vaibhav Hiremath <hvaibhav@ti.com> / Artur Siewierski <asi@datarespons.no>");
