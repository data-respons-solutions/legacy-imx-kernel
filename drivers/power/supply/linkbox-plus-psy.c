...
static s32 linkbox_ina_write(const struct i2c_client *client, u8 command, u16 value)
{
	int retries = 3;
	s32 val;
	while (retries--) {
		val = i2c_smbus_write_word_swapped(client, command, value);
		if (val == 0)
			break;
	}
	return val;
	...
	static int linkbox_plus_psy_probe(struct platform_device *pdev)
{
	...
	ret = linkbox_ina_write(priv->ina219_dcin_client, INA2XX_CONFIG, ina2xx_config.config_default);
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to write ina219 DCIN\n");
		goto cleanup;
	}
	linkbox_ina_write(priv->ina219_dcin_client, INA2XX_CALIBRATION,
	...
	ret = linkbox_ina_write(priv->ina219_manikin_client, INA2XX_CONFIG, ina2xx_config.config_default);
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to write ina219 MANIKIN\n");
		goto cleanup;
	}
	linkbox_ina_write(priv->ina219_manikin_client, INA2XX_CALIBRATION,
			  ina2xx_config.calibration_factor / ina2xx_config.shunt_uohms);
	/* Power supplies */
...
cleanup:
	if (!IS_ERR_OR_NULL(priv->ina219_dcin_client))
		i2c_unregister_device(priv->ina219_dcin_client);
	if (!IS_ERR_OR_NULL(priv->ina219_manikin_client))
		i2c_unregister_device(priv->ina219_manikin_client);
