...
static int st_lsm6dsx_init_device(struct st_lsm6dsx_hw *hw)
{
	...
	err = regmap_read(hw->regmap, ST_LSM6DSX_FUNC_SRC1_ADDR, &func_src1);
...
static int st_lsm6dsx_enable_sigmo(struct st_lsm6dsx_hw *hw, int enable)
{
	int err;
	if (enable)
		err = regmap_update_bits(hw->regmap, ST_LSM6DSX_CTRL10_C_ADDR,
...
void st_lsm6dsx_shutdown(struct device *dev)
{
	...
	if ( sensor->enable_wake) {
		...
		regmap_update_bits(hw->regmap, drdy_int_reg, BIT(6), 1);
