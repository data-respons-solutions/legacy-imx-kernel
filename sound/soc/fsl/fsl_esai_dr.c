 ...
 */
static int fsl_esai_divisor_cal(struct fsl_esai *esai_priv, bool tx, u32 ratio,
				bool usefp, u32 fp)
{
	...
	dev_dbg(&esai_priv->pdev->dev, "%s: ratio=%d, usefp=%d, fp=%d\n", __func__, ratio, usefp, fp);
	if (usefp && fp)
		goto out_fp;

	if (ratio > 2 * 8 * 256 * maxfp || ratio < 2) {
		dev_err(&esai_priv->pdev->dev, "the ratio is out of range (2 ~ %d)\n",
				2 * 8 * 256 * maxfp);
		return -EINVAL;
	} else if (ratio % 2) {
		dev_err(&esai_priv->pdev->dev, "the ratio must be even if using upper divider\n");
		return -EINVAL;
	}
	...
	if (pm == 999) {
		dev_err(&esai_priv->pdev->dev, "failed to calculate proper divisors\n");
		return -EINVAL;
	}
 ...
 */
static int fsl_esai_set_sysclk(struct fsl_esai *esai_priv, int clk_id,
				   unsigned int freq, int dir)
{
	...
	if (IS_ERR(clksrc)) {
		dev_err(&esai_priv->pdev->dev, "no assigned %s clock\n",
				clk_id % 2 ? "extal" : "fsys");
		return PTR_ERR(clksrc);
	}
	clk_rate = clk_get_rate(clksrc);
	dev_dbg(&esai_priv->pdev->dev, "%s: extal rate is %ld\n", __func__, clk_rate);
	ratio = clk_rate / freq;
	ret = clk_rate - ratio * freq;

	/* Block if clock source can not be divided into the required rate */
	if (ret != 0 && clk_rate / ret < 1000) {
		dev_err(&esai_priv->pdev->dev, "failed to derive required HCK%c rate\n",
				tx ? 'T' : 'R');
		return -EINVAL;
	}
	...
	ret = fsl_esai_divisor_cal(esai_priv, tx, ratio, false, 0);
	if (ret)
		return ret;
				   ...
				   unsigned int freq, int dir)
{
	...
	return fsl_esai_set_sysclk(esai_priv, clk_id, freq, dir);
}

/**
 * This function configures the related dividers according to the bclk rate
 */
static int fsl_esai_set_bclk(struct fsl_esai *esai_priv, bool tx, u32 freq)
{
	u32 hck_rate = esai_priv->hck_rate[tx];
	u32 sub, ratio = hck_rate / freq;

	dev_dbg(&esai_priv->pdev->dev, "%s: hck_rate = %d, freq = %d\n", __func__, hck_rate, freq);
	...
	if (sub != 0 && hck_rate / sub < 1000) {
		dev_err(&esai_priv->pdev->dev, "failed to derive required SCK%c rate\n",
				tx ? 'T' : 'R');
		return -EINVAL;
	}

	if (esai_priv->sck_div[tx] && (ratio > 16 || ratio == 0)) {
		dev_err(&esai_priv->pdev->dev, "the ratio is out of range (1 ~ 16)\n");
		return -EINVAL;
	}

	return fsl_esai_divisor_cal(esai_priv, tx, ratio, true,
			      ...
			      struct snd_soc_dai *dai)
{
	...
	ret = fsl_esai_set_bclk(esai_priv, tx, bclk);
	if (ret)
		return ret;
...
static int fsl_esai_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "%s: DR ESAI - starting clocks\n", __func__);
	/*
	 * Some platforms might use the same bit to gate all three or two of
	 * clocks, so keep all clocks open/close at the same time for safety
	 */
	...
	regmap_update_bits(esai_priv->regmap, REG_ESAI_RCCR,
			   ESAI_xCCR_xDC_MASK, ESAI_xCCR_xDC(2));
	ret = fsl_esai_set_sysclk(esai_priv, ESAI_HCKT_EXTAL, 24576000, SND_SOC_CLOCK_OUT);
	ret = fsl_esai_set_bclk(esai_priv, 1, 256*16000);
...
static const struct of_device_id dr_esai_dt_ids[] = {
	{ .compatible = "dr,esai-linkbox", },
	{}
};
MODULE_DEVICE_TABLE(of, dr_esai_dt_ids);
...
static struct platform_driver fsl_esai_driver = {
	...
	.driver = {
		.name = "fsl-esai-dr-dai",
		.owner = THIS_MODULE,
		.pm = &fsl_esai_pm_ops,
		.of_match_table = dr_esai_dt_ids,
	},
};
...
MODULE_ALIAS("platform:fsl-esai-dr-dai");
