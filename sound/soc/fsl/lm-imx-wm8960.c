...
static int hpjack_status_check(void *data)
{
	...
	if (hp_status == priv->hp_active_level) {
		...
		if (priv->iphone_jack )
			snd_soc_dapm_disable_pin(dapm, "MICB");
...
static int micjack_status_check(void *data)
{
	...
	if (mic_status == priv->mic_active_level) {
		...
		snd_soc_dapm_force_enable_pin(dapm, "MICB");
		snd_soc_dapm_force_enable_pin(dapm, "AMIC");
...
static int imx_wm8960_gpio_init(struct snd_soc_card *card)
{
	...
	if (priv->iphone_jack && gpio_is_valid(priv->mic_gpio)) {
		if (devm_gpio_request(card->dev, priv->mic_gpio, "mic-detect")) {
			dev_err(card->dev, "Failed to request mic gpio %d\n", priv->mic_gpio);
			return -ENODEV;
		}
	}
	return 0;
...
audmux_bypass:
	...
	data->is_codec_master = of_property_read_bool(pdev->dev.of_node, "codec-master");
