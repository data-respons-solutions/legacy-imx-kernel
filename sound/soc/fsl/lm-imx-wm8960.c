...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
	...
	ret = snd_soc_dai_set_sysclk(codec_dai, WM8960_SYSCLK_AUTO,
				     data->clk_frequency, SND_SOC_CLOCK_IN);
	if (ret) {
		dev_err(&data->pdev->dev, "failed to set codec sysclk: %d\n",
			ret);
		return ret;
	}
	ret = snd_soc_dai_set_pll(codec_dai, WM8960_SYSCLK_AUTO, 0,
				  data->clk_frequency, data->clk_frequency);
	if (ret) {
		dev_err(&data->pdev->dev, "failed to set codec PLL: %d\n", ret);
		return ret;
	}
